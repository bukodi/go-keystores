...
func (ks *Pkcs11KeyStore) aquireSession() (*Pkcs11Session, error) {
	...
	pin, err := ks.driverConfig.pkcs11Provider.PINAuthenticator(ks.Name(), "", false)
	if err != nil {
		return nil, keystores.ErrorHandler(err, ks)
	}

	if err = ks.driverConfig.pkcs11Ctx.Login(s.hSession, p11api.CKU_USER, pin); err != nil {
